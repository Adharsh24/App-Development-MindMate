...
const LandingPage = () => {
  ...
  const handleSubmitMood = async () => {
    if (rating >= 1 && rating <= 10) {
      ...
      try {
        const response = await fetch('http://localhost:8080/describeMood', {
          method: 'POST',
        ...
        console.error('Error fetching mood description:', error);
      }
    }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      ...
      <Container style={{ flex: 1 }}>
        ...
        <Paper elevation={3} style={{ padding: '3rem', marginBottom: '3rem', backgroundColor: '#cccccc', textAlign: 'center' }}>
        ...
        <Paper elevation={3} style={{ padding: '2rem', marginBottom: '2rem', backgroundColor: '#f4e1d2' }}>
          ...
          {description && (
            <Typography variant="body1" style={{ marginTop: '1rem', color: '#333' }}>
              {description}
            </Typography>
          )}
        </Paper>
